...
bool BestImprovementSwap(Instance& instance, Solution& solution)   // Deivid
{
    ...
    while(!neighborhood_structures.empty()){
        ...
        switch(neighborhood_structures[rand_nh_num]){
            ...
            case TWO_OPT:
                ...
                break;
            case OR_OPT:
                has_solution_improved = BestImprovementOrOpt(instance, curr_solution);   //TODO: here too
                break;
        }
...
Solution IteratedLocalSearch(int max_iters, int max_iters_ILS, Instance& instance)
{
    ...
    for(int i = 0; i < max_iters; i++){
        ...
        while(curr_iter_counter_ILS <= max_iters_ILS){
            ...
            // Compare curr iter's solution post-local-search to current all-ILS-up-to-now execs best solution
            // If it's lower, make it the new best solution and restart ILS
            // Same logic as the RVND. If it's improved within current ILS, it has more room to improve
            // Means it only doesn't become the new best if it doesn't improve AT ALL once in all max_iters_ILS execs
        ...
        // Now after after 1 full ILS execution (Executing LocalSearchRVND() max_iters_ILS times)
        // Check if it produced a better solution than previous ILS execution, attributing if so
...
int main(int argc, char* argv[])
{
    ...
    // TODO: Check research papers to see if there's a better rule on choosing max_iters_ILS for this specific problem
    ...
    for(int i = 0; i < 10; i++){
        ...
        cout << "Current Solution Fee (iter " << i + 1 << "): " << solution.getSolutionFee() << '\n';
    }
