...
std::string ExtractInstanceName(const std::string &text) {
  ...
  if (text == "instances\\ex_instance.txt" ||
      text == ".\\instances\\ex_instance.txt") {
    return "ex_instance";
  }
...
void PrintCurrentIteration(int iteration,
                           DoublyLinkedList curr_solution_sequence,
...
int main(int argc, char *argv[]) {
  ...
  std::map<std::string, int32_t> optimal_values_map = {
      {"ex_instance", 360}, {"n60A", 453},    {"n60B", 1757},
      {"n60C", 0},          {"n60D", 69102},  {"n60E", 58935},
      {"n60F", 93045},      {"n60G", 64632}, // May be lower
      {"n60H", 145007},     {"n60I", 43286},  {"n60J", 53555},
      {"n60K", 458752},     {"n60L", 332941}, {"n60M", 516926},
      {"n60N", 492572},     {"n60O", 527459}, // May be lower
      {"n60P", 396183}};
  ...
  for (int i = 0; i < 10; i++) {
    ...
    benchmarker.metaheuristic_start_time =
        std::chrono::high_resolution_clock::now();
    solution = IteratedLocalSearch(max_iters, max_iters_ILS, instance);
    benchmarker.metaheuristic_end_time =
        std::chrono::high_resolution_clock::now();

    benchmarker.metaheuristic_avg_elapsed_time +=
        (benchmarker.metaheuristic_end_time -
         benchmarker.metaheuristic_start_time);
    ...
    if (i == 0) {
      ...
      benchmarker.best_solution_sequence = solution.fruit_order;
