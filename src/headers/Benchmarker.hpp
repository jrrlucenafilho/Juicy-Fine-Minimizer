...
struct benchmarker_t {
  ...
  DoublyLinkedList best_solution_sequence;
  ...
  std::chrono::time_point<std::chrono::high_resolution_clock>
      constr_heuristic_start_time;
  std::chrono::time_point<std::chrono::high_resolution_clock>
      constr_heuristic_end_time;
  ...
  // Metaheuristic time
  std::chrono::time_point<std::chrono::high_resolution_clock>
      metaheuristic_start_time;
  std::chrono::time_point<std::chrono::high_resolution_clock>
      metaheuristic_end_time;
  ...
  void printResults(std::string instance_name, int32_t optimal_cost) {
    ...
    std::cout << "--------------------------Constructive "
                 "Heuristic-----------------------------\n";
    std::cout << "Constructive Heuristic Avg Cost: "
              << constructive_heuristic_avg_cost << '\n';
    std::cout << "Constructive Heuristic Avg Time: "
              << constructive_heuristic_avg_elapsed_time.count() << " s \n";
    std::cout << "Constructive Heuristic GAP: "
              << CalculateGap(constructive_heuristic_avg_cost, optimal_cost)
              << " %\n";
    std::cout << "----------------------------------RVND-----------------------"
                 "----------------\n";
    ...
    std::cout << "Metaheuristic Avg Cost: " << metaheuristic_solution_avg_cost
              << '\n';
    std::cout << "Metaheuristic Avg Time: "
              << metaheuristic_avg_elapsed_time.count() << " s \n";
    std::cout << "Metaheuristic GAP: "
              << CalculateGap(metaheuristic_solution_avg_cost, optimal_cost)
              << " %\n";
